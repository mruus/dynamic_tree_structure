...
var foundNodes = [];
var activeFoundNode = 0;
// Function to create a tree node element
function createTreeNode(node, collapse) {
    ...
    if (node.children.length > 0) {
        var nodeToggle = document.createElement("div");
        // Style node toggle
    ...
    treeNode.classList.add("tree__node", node.children.length > 0 ? "children__node" : "no__children__node");
    return treeNode;
}
// Recursive function to render the tree
function renderTree(treeData, treeContainer, collapse) {
    if (collapse === void 0) { collapse = false; }
    treeData.forEach(function (node, index) {
        // Create a tree node element
        var treeNode = createTreeNode(node, collapse);
...
if (treeData.length > 0) {
    ...
    var treeContainer = document.querySelector(".tree__container");
...
function collapse_or_expand(expand, element) {
    if (element === void 0) { element = null; }
    var parentContainer = $(".parent__container");
    if (parentContainer.length > 0) {
        var element_1 = parentContainer[0];
        var children = $(element_1).children("li:has(.toggle__node)");
        children.map(function (index, item) {
            var parentItem = $(item).children(".parent__container");
            coe(parentItem, expand);
        });
    }
}
function coe(parent, expand) {
    var subNodes = $(parent).children("li:has(.toggle__node)");
...
function highlightNode(treeNode) {
...
function moveDown() {
    if (activeFoundNode === foundNodes.length - 1)
        activeFoundNode = -1;
    activeFoundNode++;
    removeHighlighting();
    $(".current__find").text(activeFoundNode + 1);
...
function moveUp() {
    if (activeFoundNode === 0)
        activeFoundNode = foundNodes.length;
    activeFoundNode--;
    removeHighlighting();
    $(".current__find").text(activeFoundNode + 1);
    highlightNode(foundNodes[activeFoundNode]);
...
function scrollToElement() {
    var targetElement = $(".highlighted__node");
    if (targetElement) {
